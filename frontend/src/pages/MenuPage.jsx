...
const AdminPage = () => {
    ...
    const [showManualOrder, setShowManualOrder] = useState(null);
    ...
    const fetchInitialData = async () => {
        ...
        // Fetch existing items first before seeding
        const { data: existingItems } = await supabase.from('menu_items').select('*');

        await Promise.all([fetchTables(), fetchOrders(), fetchCategories(), fetchCustomers()]);


        if (existingItems) {
            for (const drink of requiredDrinks) {
                if (!existingItems.find(m => m.name.toLowerCase() === drink.name.toLowerCase())) {
    ...
    useEffect(() => {
        ...
        const pollInterval = setInterval(() => {
            fetchTables();
            fetchOrders();
        }, 15000); // ✅ Increased from 1500ms to 15s to reduce server load
    ...
    const clearAllData = async () => {
        ...
        try {
            // ✅ FIX: Use .gte('id', 0) to include takeaway orders with table_id 0
            const { error: orderError } = await supabase.from('orders').delete().gte('id', 0);
    ...
    // ✅ FIX 2: rejectOrder now also frees the table
    const rejectOrder = async (orderId) => {
        try {
            ...
            // Free the table if it's a real table (not takeaway)
            if (targetOrder.table_id && targetOrder.table_id > 0) {
                ...
                // Update local state immediately so floor view updates without waiting for poll
    ...
    // ✅ FIX 1: Added isParcel as 4th parameter so findIndex works correctly
        ...
        <>
            <div className="admin-container animate-fade" style={{ minHeight: '100vh', paddingBottom: '40px' }}>
                ...
                <div style={{
                    ...
                    {[
                        ...
                        { label: 'Active', val: stats.activeOrders, icon: '📋', c: 'var(--accent-purple)' },
                ...
                {activeTab === 'customize' ? (
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '24px' }}>
                        ...
                        <div className="glass" style={{ padding: '32px', borderRadius: '24px' }}>
                            ...
                            <form onSubmit={async (e) => {
                                ...
                                const newItem = {
                                    ...
                                    price: Math.max(0, parseFloat(formData.get('price')) || 0),
                            ...
                            <input
                                ...
                                className="glass"
                                style={{ width: '100%', padding: '12px 16px', borderRadius: '16px', color: 'var(--text-main)', outline: 'none', marginBottom: '16px' }}
                            />
                        ...
                        <div className="glass" style={{ padding: '32px', borderRadius: '24px' }}>
                            ...
                            <button
                                ...
                                style={{ width: '100%', marginTop: '20px', padding: '16px', backgroundColor: 'var(--glass)', color: 'var(--text-main)', border: '1px solid var(--border-subtle)', borderRadius: '16px', fontWeight: '700', cursor: 'pointer' }}
                            >
                            ...
                            <button
                                ...
                                style={{ width: '100%', marginTop: '12px', padding: '18px', backgroundColor: 'var(--accent-white)', color: 'var(--bg-dark)', fontWeight: '800', borderRadius: '18px', cursor: 'pointer', boxShadow: '0 8px 32px rgba(255,255,255,0.1)', letterSpacing: '-0.01em' }}
                            >
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                        <button
                            ...
                            style={{ padding: '20px', background: 'var(--accent-white)', color: 'var(--bg-dark)', borderRadius: '24px', fontWeight: '800', fontSize: '1rem', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px', boxShadow: '0 8px 32px rgba(255,255,255,0.1)', marginBottom: '8px' }}
                        >
                        ...
                        {orders.filter(o => o.table_id === 0 && o.status !== 'paid' && o.status !== 'rejected').map(order => {
                            ...
                            const displayItems = order.items.filter(i => i.type !== 'METADATA' && i.type !== 'PAYMENT_METADATA');
                                ...
                                <div key={order.id} className="glass" style={{ padding: '24px', borderRadius: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <div style={{ flex: 1 }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '12px' }}>
                                            ...
                                            <span style={{
                                                padding: '4px 12px', borderRadius: '16px', fontSize: '0.75rem', fontWeight: '700', letterSpacing: '0.05em',
                                    ...
                                    <div style={{ textAlign: 'right', display: 'flex', flexDirection: 'column', alignItems: 'flex-end', marginLeft: '24px' }}>
                                        ...
                                        <div style={{ display: 'flex', gap: '12px' }}>
                                            {order.status === 'new' ? (
                                                <button onClick={() => acceptOrder(order.id, 0)} style={{ padding: '8px 20px', fontSize: '0.9rem', backgroundColor: 'var(--accent-white)', color: 'var(--bg-dark)', borderRadius: '16px', fontWeight: '700' }}>
                                                    Ready
                                                </button>
                                            ) : (
                                                <button onClick={() => completeTakeaway(order.id)} style={{ padding: '8px 20px', fontSize: '0.9rem', backgroundColor: '#4ade80', color: '#000', borderRadius: '16px', fontWeight: '700' }}>
                                                    Handover
                                                </button>
                                            )}
                                            <button onClick={(e) => { e.stopPropagation(); setInvoiceOrder(order); }} style={{ padding: '8px 20px', fontSize: '0.9rem', backgroundColor: 'var(--glass)', border: '1px solid var(--border-subtle)', color: 'var(--text-main)', borderRadius: '16px', fontWeight: '600' }}>
                                                Invoice
                                            </button>
                                        </div>
                                    </div>
                        ...
                        {orders.filter(o => o.table_id !== 0 && o.status !== 'paid' && o.status !== 'rejected').map(order => (
                            <div key={order.id} className="glass" style={{ padding: '24px', borderRadius: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <div>
                                    ...
                                    <p style={{ color: 'var(--text-muted)', fontSize: '1rem', fontWeight: '500' }}>
                                        {order.items.filter(i => i.type !== 'METADATA' && i.type !== 'PAYMENT_METADATA').map(i => `${i.qty}x ${i.name}`).join(', ')}
                                    </p>
                                </div>
                ...
                {selectedTableOrder && (
                    <div className="animate-overlay" style={{
                        ...
                        WebkitBackdropFilter: 'blur(12px)', zIndex: 2000,
                                ...
                                <div style={{ marginTop: '20px', borderTop: '1px dashed var(--border-subtle)', paddingTop: '20px' }}>
                                    {!isAddingItem ? (
                                        <button
                                            ...
                                            style={{ width: '100%', padding: '12px', backgroundColor: 'rgba(255,255,255,0.05)', border: '1px solid var(--border-subtle)', borderRadius: '12px', color: 'var(--text-main)', fontWeight: '600', cursor: 'pointer' }}
                                        >
                                            + Add Item
                                        </button>
                                    ) : (
                                                    ...
                                                    {menuItems
                                                        ...
                                                        .map(item => (
                                                            <button
                                                                ...
                                                                style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 16px', backgroundColor: 'rgba(255,255,255,0.03)', border: '1px solid var(--border-subtle)', borderRadius: '12px', color: 'white', textAlign: 'left', cursor: 'pointer' }}
                                                            >
            ...
            {qrTable && (
                <div className="animate-overlay" style={{
                    ...
                    WebkitBackdropFilter: 'blur(12px)', zIndex: 3000,
                    display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px'
                }}>
                    <div className="glass animate-fade" style={{ width: '100%', maxWidth: '430px', borderRadius: '32px', padding: '40px', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '32px', backgroundColor: 'var(--bg-surface)', position: 'relative', maxHeight: '90vh', overflowY: 'auto' }}>
                        <button onClick={() => setQrTable(null)} style={{ position: 'absolute', top: '20px', right: '20px', background: 'var(--glass)', border: '1px solid var(--border-subtle)', width: '36px', height: '36px', borderRadius: '18px', color: 'white', cursor: 'pointer', zIndex: 10 }}>✕</button>

                        <div id="qr-to-print" style={{ position: 'relative', width: '320px', boxShadow: '0 20px 50px rgba(0,0,0,0.2)', overflow: 'hidden', backgroundColor: '#1E847F', fontSize: '16px', paddingBottom: '480px' }}>
                            <div className="qr-card-content" style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'space-between', padding: '13% 10%', boxSizing: 'border-box' }}>
                                <div style={{ position: 'absolute', top: '4%', right: '4%', bottom: '4%', left: '4%', border: '3px solid #D4AF37', pointerEvents: 'none', zIndex: 0 }} />
                                <div style={{ position: 'absolute', inset: 0, zIndex: 0, opacity: 0.1, backgroundImage: 'url("data:image/svg+xml,%3Csvg viewBox=%220 0 200 200%22 xmlns=%22http://www.w3.org/2000/svg%22%3E%3Cfilter id=%22noiseFilter%22%3E%3CfeTurbulence type=%22fractalNoise%22 baseFrequency=%220.65%22 numOctaves=%223%22 stitchTiles=%22stitch%22/%3E%3C/filter%3E%3Crect width=%22100%25%22 height=%22100%25%22 filter=%22url(%23noiseFilter)%22/%3E%3C/svg%3E")' }} />
                                <h1 style={{ color: '#D4AF37', fontFamily: '"Times New Roman", Times, serif', fontSize: '180%', fontWeight: 'bold', letterSpacing: '2px', margin: '0', textShadow: '2px 2px 4px rgba(0,0,0,0.5)', zIndex: 1, textAlign: 'center' }}>KBC DAWAT</h1>
                                <div style={{ width: '58%', aspectRatio: '1', backgroundColor: '#ffffff', borderRadius: '16px', border: '4px solid #D4AF37', boxShadow: '0 8px 16px rgba(0,0,0,0.4)', zIndex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '5%', boxSizing: 'border-box' }}>
                                    <QRCodeSVG
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%', gap: '8px', zIndex: 1 }}>
                                    <div style={{ color: '#D4AF37', fontFamily: '"Brush Script MT", "Great Vibes", cursive', fontSize: '200%', textShadow: '1px 1px 2px rgba(0,0,0,0.5)', textAlign: 'center', fontStyle: 'italic' }}>Scan to Order</div>
                                    <div style={{ width: '70%', height: '1.5px', backgroundColor: '#D4AF37', boxShadow: '0 1px 2px rgba(0,0,0,0.3)' }} />
                                    <div style={{ color: '#D4AF37', fontFamily: '"Times New Roman", Times, serif', fontSize: '110%', letterSpacing: '1px', textAlign: 'center' }}>www.kbcdawat.com</div>
                                </div>
                                <div style={{ color: '#D4AF37', fontFamily: 'sans-serif', fontSize: '60%', letterSpacing: '0.08em', zIndex: 1, textAlign: 'center', fontWeight: 'bold', lineHeight: '1.4' }}>
                                    <div>FOOOD WEB</div>
                        ...
                        <button
                            ...
                            onClick={() => {
                                ...
                                style.innerHTML = `@media print { body * { visibility: hidden !important; } #qr-to-print, #qr-to-print * { visibility: visible !important; } #qr-to-print { position: absolute !important; left: 0 !important; top: 0 !important; margin: 0 !important; padding: 0 !important; width: 100mm !important; height: 150mm !important; box-shadow: none !important; border: none !important; print-color-adjust: exact !important; -webkit-print-color-adjust: exact !important; display: block !important; } }`;
                    ...
                    <div className="glass" style={{ width: '100%', maxWidth: '480px', borderRadius: '24px', padding: '32px' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '24px', maxHeight: '300px', overflowY: 'auto', paddingRight: '8px' }}>
                            ...
                            <div style={{ display: 'flex', gap: '8px', marginTop: '8px' }}>
                                <select className="glass" value={newMatId} onChange={(e) => setNewMatId(e.target.value)} style={{ flex: 1, padding: '12px', borderRadius: '12px', color: 'var(--text-main)', fontSize: '0.85rem' }}>
                                    <option value="">Select Material</option>
                                    {materials.map(m => <option key={m.id} value={m.id} style={{ color: 'black' }}>{m.name}</option>)}
                                </select>
                                <input type="number" placeholder="Qty" value={newMatQty} onChange={(e) => setNewMatQty(e.target.value)} className="glass" style={{ width: '80px', padding: '12px', borderRadius: '12px', color: 'var(--text-main)', fontSize: '0.85rem' }} />
                                <button
                                    onClick={() => {
                    ...
                    <div className="glass" style={{ width: '100%', maxWidth: '900px', height: '90vh', borderRadius: '32px', padding: '32px', display: 'flex', flexDirection: 'column' }}>
                        ...
                        <div style={{ flex: 1, display: 'flex', gap: '24px', overflow: 'hidden' }}>
                            ...
                            <div style={{ flex: 2, display: 'flex', flexDirection: 'column', gap: '16px', overflowY: 'auto', paddingRight: '8px' }}>
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                    <input type="text" placeholder="🔍 Search items by name..." value={manualSearch} onChange={(e) => setManualSearch(e.target.value)} className="glass" style={{ width: '100%', padding: '14px 20px', borderRadius: '16px', color: 'white', border: '1px solid var(--border-subtle)', outline: 'none' }} />
                                    <div style={{ display: 'flex', gap: '8px', overflowX: 'auto', paddingBottom: '8px', msOverflowStyle: 'none', scrollbarWidth: 'none' }}>
                                        {['all', ...categories.map(c => c.name)].map(catName => (
                                            <button key={catName} onClick={() => setManualCategory(catName)} style={{ padding: '8px 20px', backgroundColor: manualCategory === catName ? 'var(--accent-white)' : 'var(--glass)', color: manualCategory === catName ? 'var(--bg-dark)' : 'white', border: '1px solid var(--border-subtle)', borderRadius: '20px', fontSize: '0.8rem', fontWeight: '700', whiteSpace: 'nowrap', transition: 'all 0.2s' }}>
                                                {catName.toUpperCase()}
                                            </button>
                                        ))}
                                    </div>
                                    ...
                                    {manualCart.map((item, idx) => (
                                        <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                            ...
                                            <div style={{ display: 'flex', gap: '8px' }}>
                                                <button onClick={() => { setManualCart(prev => prev.map(i => i.id === item.id ? { ...i, qty: Math.max(0, i.qty - 1) } : i).filter(i => i.qty > 0)); }} style={{ width: '24px', height: '24px', borderRadius: '12px', border: '1px solid var(--border-subtle)', color: 'white' }}>-</button>
                                                <button onClick={() => { setManualCart(prev => prev.map(i => i.id === item.id ? { ...i, qty: i.qty + 1 } : i)); }} style={{ width: '24px', height: '24px', borderRadius: '12px', border: '1px solid var(--border-subtle)', color: 'white' }}>+</button>
                                            </div>
                                        </div>
                                    ))}
                                </div>
